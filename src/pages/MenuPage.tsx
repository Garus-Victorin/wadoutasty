...
import { Utensils, Star, Info, ChevronRight } from 'lucide-react';
    ...
    <div className="pt-20 min-h-screen bg-background">
      ...
      <section className="relative py-24 px-6 lg:px-12 bg-foreground overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto flex flex-col items-center text-center gap-6">
          ...
          >
            <span className="text-xs font-bold uppercase tracking-[0.4em] text-primary">Saveurs Béninoises</span>
            ...
            <p className="text-white/60 max-w-2xl mx-auto mt-6 text-lg leading-relaxed">
              Une sélection rigoureuse de plats traditionnels préparés avec des ingrédients frais et des épices locales pour une expérience authentique.
            </p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {[1, 2, 3, 4, 5, 6].map(i => (
                <div key={i} className="aspect-[4/5] bg-muted animate-pulse rounded-[32px]" />
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                ...
                >
                  <Card className="group bg-white rounded-[32px] overflow-hidden border border-border/50 shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
                    ...
                    <CardContent className="p-8 flex flex-col gap-4">
                      ...
                      <Button className="w-full mt-4 h-12 rounded-xl font-bold group">
          ...
          {!loading && filteredItems.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 gap-6 opacity-50">
              ...
              <p className="text-xl font-display font-bold">Aucun plat dans cette catégorie pour le moment.</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="py-12 px-6 lg:px-12 bg-primary/5 border-y border-primary/10">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex items-center gap-6 text-center md:text-left">
            ...
            <div className="flex flex-col gap-1">
              <h4 className="text-lg font-display font-bold">Allergies ou Régimes Spéciaux ?</h4>
              <p className="text-sm text-muted-foreground">N'hésitez pas à en informer notre personnel lors de votre commande.</p>
            </div>
          </div>
